...
export function LoginView() {
  ...
  const [email, setEmail] = useState("user71@example.com");
  const [password, setPassword] = useState("12345678");

  const canSubmit = useMemo(() => email.trim().length > 3 && password.trim().length > 0, [email, password]);

  const handleLogin = async () => {
    dispatch(authActions.loginRequested({ email: email.trim(), password: password.trim() }));
  };
          ...
          <CardContent sx={{ p: 4 }}>
            ...
            <Box sx={{ mt: 3, display: "flex", flexDirection: "column", gap: 2.5 }}>
              ...
              <TextField
                ...
                size="medium"
                disabled={isLoading}
                error={!!error && password.trim().length === 0}
                sx={{
                  "& .MuiOutlinedInput-root": {
                    borderRadius: 2,
                  },
            ...
            <Box sx={{ display: "flex", justifyContent: "space-between", gap: 2, flexWrap: "wrap" }}>
              ...
              <Link href={routes.register} underline="hover" color="text.secondary">
                Create an account
              </Link>
            </Box>
          </CardContent>
