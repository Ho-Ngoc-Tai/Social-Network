...
const sendFriendRequest = async (userId: string, token?: string) => {
  try {
    const baseUrl = process.env.API_BASE_URL || "https://social-backend.bijancob.io.vn";
    const fullUrl = `${baseUrl}/users/${encodeURIComponent(userId)}/friend`;
...
export async function POST(
  ...
  try {
    ...
    const resp = await sendFriendRequest(userId, token);
