...
const getMessages = async (conversationId: string, token?: string) => {
  try {
    const baseUrl = process.env.API_BASE_URL || "https://social-backend.bijancob.io.vn";
    const fullUrl = `${baseUrl}/conversations/${conversationId}/messages`;
...
export async function GET(
  ...
  try {
    ...
    const resp = await getMessages(conversationId, token);
