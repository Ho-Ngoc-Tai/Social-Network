...
const markMessagesAsRead = async (userId: string, token?: string) => {
  try {
    const baseUrl = process.env.API_BASE_URL || "https://social-backend.bijancob.io.vn";
    const fullUrl = `${baseUrl}/conversations/thread/${encodeURIComponent(userId)}/read`;
...
export async function POST(
  ...
  try {
    ...
    const resp = await markMessagesAsRead(userId, token);
