...
const openConversation = async (userId: string, token?: string) => {
  try {
    const baseUrl = process.env.API_BASE_URL || "https://social-backend.bijancob.io.vn";
    const fullUrl = `${baseUrl}/conversations/open`;
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!userId) {
      return Response.json(
        { error: "Bad Request", message: "Missing user_id in request body" },
        { status: 400 },
      );
    }

    const resp = await openConversation(userId, token);
