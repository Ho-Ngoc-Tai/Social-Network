...
import { createMockPost, getMockFeed } from "@/app/services/mockData";

function* loadFeedWorker(): Generator {
  yield delay(350);
  const items = getMockFeed();
  yield put(feedActions.loadFeedSucceeded({ items }));
}

function* createPostWorker(action: ReturnType<typeof feedActions.createPostRequested>): Generator {
  yield delay(250);
